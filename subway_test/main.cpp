...
int connect_to_hostname(char **hostname)
{
  ...
  info.ai_protocol = SOCK_STREAM;
  getaddrinfo("datamine.mta.info", "80", &info, &result);

  if (!connect(s, result->ai_addr, sizeof(struct sockaddr_in))) {
...
int main(int argc, char **argv)
{
  ...
  FILE *sockfile = (FILE *) fdopen(s, "r");
  pb_istream_t istream = {&pb_istream_callback, &s, SIZE_MAX};
